...
/**
 * PayPal Shop Maker 4
 * (C) 2008-2010 e.World Technology Limited. All rights reserved.
 */
...
function ewpp_LoadMenu(&$rs) {
	...
	global $ewpp_MenuPageUrl, $ewpp_MenuId, $ewpp_MenuLink, $ewpp_MenuGen,
	...
	if ($rs && !$rs->EOF) {
		...
		$ewpp_MenuFn = $ewpp_MenuPageUrl . "?" . EWPP_MENU_ID . "=" . $ewpp_MenuId;
...
function ewpp_LoadCat(&$rs) {
	...
	global $ewpp_CatId, $ewpp_CatName, $ewpp_CatPath, $conn;
	if ($rs && !$rs->EOF) {
		$ewpp_CatId = $rs->fields("CategoryId");

		//$ewpp_CatFn = $ewpp_CartUrl . "?" . EWPP_CATEGORY_ID . "=" . $ewpp_CatId;
...
function ewpp_ImageName($fn, $imgtype) {
	...
	if ($wrkext == "swf") { // swf
		$fn = str_replace(" ", "_", $path_parts['basename']);
	} else { // Assume image
		$fn = str_replace(" ", "_", $imgtype . $path_parts['basename']);
	}
	return $fn;
}
...
// Get image tag
function ewpp_ImageTag($id, $fn, $imgtype, $width, $height) {
	...
	if ($wrkfn <> "") {
		...
		if ($wrkext == "swf") { // handle swf
			$wrkid = "swf_" . str_replace(" ", "_", $id);
			if ($width <= 0)
				$width = EWPP_SWF_DEFAULT_WIDTH;
			if ($height <= 0)
				$height = EWPP_SWF_DEFAULT_HEIGHT;
			$args = EWPP_SWF_ARGS; // embedSWF() arguments from version
			...
			if ($info[0] > 0 && $info[1] > 0) {
				...
				if ($EWPP_PAGE_ID == "view" && EWPP_SWF_GET_SIZE_VIEW) {
					$width = $info[0];
					$height = $info[1];
				}
			}
			return "<div id=\"" . $wrkid . "\">" . $wrkfn . "</div>" .
				"<script type=\"text/javascript\">" .
				"swfobject.embedSWF(\"" . $wrkfn . "\", \"" . $wrkid . "\", \"" . $width . "\", \"" . $height . "\", \"" . $args . "\");" .
				"</script>";
		} else { // assume image
			return "<img border=\"0\" alt=\"\" src=\"" . $wrkfn ."\" >";
		}
	} else {
		return "";
	}
...
function ewpp_SelectView($name, $data, $type, $size, $def) {
	...
	global $PPLanguage;
	$outstr = "<select name=\"" . $name . "\" onchange=\"UpdatePrice(this.form);\"";
	...
	if ($cnt > 0) {
		...
		for ($i=0; $i<$cnt; $i++) {
			if ($i == 0 || ($i > 1 && $i % $col == 0))
				$outstr .= "<tr>";
			$arOption = explode("=", $arOptions[$i]); 
			$outstr .= "<td><label><input type=\"" . $type . "\" onclick=\"UpdatePrice(this.form);\"";
...
function ewpp_LoadAlpha() {
	...
	global $ewpp_Alpha, $ewpp_PagingIndexes, $EWPP_PRODUCT_ALPHANUMERIC_INDEX;
...
function ewpp_BuildPagingIndex($filter) {
	...
	global $conn, $ewpp_PagingIndexes, $EWPP_PRODUCT_ALPHANUMERIC_INDEX;
	...
	for ($i = 0; $i < strlen($EWPP_PRODUCT_ALPHANUMERIC_INDEX); $i++) {
		...
		$rswrk = $conn->Execute(ewpp_PagingSql($sAlpha, $filter));
		$ewpp_PagingIndexes[] = $rswrk->fields[0];
		$rswrk->Close();
	}
}
...
function ewpp_PagingSqlFilter($alpha) {
	...
	global $EWPP_PRODUCT_ALPHANUMERIC_INDEX;
	if ($alpha == "~") {
		$sWrk = "";
		for ($i = 0; $i < strlen($EWPP_PRODUCT_ALPHANUMERIC_INDEX); $i++) {
			$sAlpha = substr($EWPP_PRODUCT_ALPHANUMERIC_INDEX, $i, 1);
...
// Get custom queries // 4.0
function ewpp_CustomQueries($name) {

	global $conn;
	$wrkstr = "[";
	if (defined("EWPP_" . strtoupper($name) . "_SQL")) {

		// Execute query
		$rs = $conn->Execute(constant("EWPP_" . strtoupper($name) . "_SQL"));
	...
	$wrkstr .= "]";
	$_SESSION[EWPP_PROJECT_NAME . "_" . $name] = $wrkstr;
	return $wrkstr;
}
...
class cpMenu {
	...
	var $IsRoot = FALSE;
	var $NoItem = NULL;
	var $ItemData = array(); // data type: array of cpMenuItem
	...
	// Add menu item
	function AddMenuItem($id, $text, $gen, $fn, $url, $parentid) {

		$item = new cpMenuItem($id, $text, ($gen)?$fn:$url, $parentid);
		if ($item->ParentId < 0) {
			$this->AddItem($item);
		} else {
	...
	function Render() {
		...
		if ($this->IsRoot)
			echo " class=\"" . EWPP_MENUBAR_CLASSNAME . "\"";
		echo ">\n";
		foreach ($this->ItemData as $item) {
			if ($this->RenderItem($item)) {
				echo "<li><a";
				if (!is_null($item->SubMenu) && $item->SubMenu->RenderMenu())
					echo " class=\"" . EWPP_MENUBAR_SUBMENU_CLASSNAME . "\"";
				if ($item->Url <> "")
...
class cpMenuItem {
	...
	//  Constructor
	function cpMenuItem($id, $text, $url, $parentid) {
	...
	function AddItem($item) {
		...
		if (is_null($this->SubMenu))
			$this->SubMenu = new cpMenu($this->Id);
		$this->SubMenu->AddItem($item);
	}
}

// Output menu items
function ewpp_WriteMenu() {

	global $conn, $ewpp_MenuId, $ewpp_MenuLink, $ewpp_MenuGen, $ewpp_MenuFn, $ewpp_MenuUrl, $ewpp_MenuParentId;
	$oMenuRoot = new cpMenu(EWPP_MENUBAR_ROOTMENU_ID);
...
function ewpp_WriteCategories() {

	global $PPLanguage, $conn, $ewpp_CartUrl;
	$oCatRoot = new cpMenu(EWPP_MENUBAR_ROOTCAT_ID);
	...
	if ($rscat = $conn->Execute($sql_cat)) {
		if (EWPP_SHOW_ALL_PRODUCTS_CATEGORY || $rscat->RecordCount() <= 0) {
			...
			$oCatRoot->AddMenuItem(-1, $catname, TRUE,
				$ewpp_CartUrl . "?" . EWPP_COMMAND . "=" .EWPP_COMMAND_RESETALL, "", -1);
		}
		while (!$rscat->EOF) {
			...
			if (EWPP_SHOW_EMPTY_CATEGORY || $cnt > 0)
				$oCatRoot->AddMenuItem($catid, $catname, TRUE,
					 $ewpp_CartUrl . "?" . EWPP_CATEGORY_ID . "=" . $rscat->fields("CategoryId"),
...
function ewpp_AddFilters(&$where) {
	...
	if (EWPP_PRODUCT_DEFAULT_FILTER <> "") {
		if ($where <> "") $where .= " AND ";
		$where .= "(" . EWPP_PRODUCT_DEFAULT_FILTER . ")";
	}
	...
	if (EWPP_USE_ITEM_COUNT && EWPP_PRODUCT_ITEMCOUNT_FILTER <> "") {
		if ($where <> "") $where .= " AND ";
		$where .= EWPP_PRODUCT_ITEMCOUNT_FILTER;
	}
}
...
function ewpp_WriteScript() {
	...
	global $PPLanguage, $EWPP_DISCOUNT_CODE, $EWPP_PAGE_ID;
	$str = "var EWPP_PAGE_ID='" . $EWPP_PAGE_ID . "';"; 
	$str .= "var EWPP_SESSION_ID='" . ewpp_TeaEncrypt(session_id(), EWPP_RANDOM_KEY) . "';";
	$str .= "var EWPP_CUSTOM_AS_TEXTAREA=" . ((EWPP_CUSTOM_AS_TEXTAREA) ? 1 : 0) . ";";
	if (in_array($EWPP_PAGE_ID, array("checkout", "shipping", "confirm"))) {
		$EWPP_DISCOUNT_CODE = ewpp_UseDiscountCode();
		$str .= "var ewpp_countryList=" .ewpp_CustomQueries("CountryList") . ";\n"; // Country details
		$str .= "var ewpp_stateList=" . ewpp_CustomQueries("StateList") . ";\n"; // State details
		$str .= "var ewpp_discountList=" . ewpp_CustomQueries("DiscountList") . ";\n"; // Discount details
		$str .= "var ewpp_discountTypeList=" . ewpp_CustomQueries("DiscountTypeList") . ";\n";
		$str .= "var ewpp_shipMethodList=" . ewpp_CustomQueries("ShipMethodList") . ";\n"; // Shipping cost details
		$str .= "var ewpp_shipcostList0=" . ewpp_CustomQueries("ShipcostList0") . ";\n";
		$str .= "var ewpp_shipcostList1=" . ewpp_CustomQueries("ShipcostList1") . ";\n";
		$str .= "var ewpp_shipcostList2=" . ewpp_CustomQueries("ShipcostList2") . ";\n";
		$str .= "var ewpp_shipTypeList=" . ewpp_CustomQueries("ShipTypeList") . ";\n"	;
		$str .= "var ewpp_taxList=" . ewpp_CustomQueries("TaxList") . ";\n"; // Tax details
	} else {
		$EWPP_DISCOUNT_CODE = FALSE;
		$str .= "var ewpp_countryList=[];";
		$str .= "var ewpp_stateList=[];";
		$str .= "var ewpp_discountList=[];";
		$str .= "var ewpp_discountTypeList=[];";
		$str .= "var ewpp_shipMethodList=[];";
		$str .= "var ewpp_shipcostList0=[];";
		$str .= "var ewpp_shipcostList1=[];";
		$str .= "var ewpp_shipcostList2=[];";
		$str .= "var ewpp_shipTypeList=[];"	;
		$str .= "var ewpp_taxList=[];";
	}
	$str .= "var EWPP_DISCOUNT_CODE=" . (($EWPP_DISCOUNT_CODE) ? 1 : 0) . ";";
	$str .= "var EWPP_USE_PAYPAL=" . ((EWPP_USE_PAYPAL) ? 1 : 0) . ";";

	//$str .= "var EWPP_PAYPAL_URL='" . EWPP_PAYPAL_URL . "';";
	$str .= "var EWPP_DEFAULT_CURRENCY_SYMBOL='" . EWPP_DEFAULT_CURRENCY_SYMBOL . "';";
	$str .= "var EWPP_DEFAULT_MON_THOUSANDS_SEP='" . EWPP_DEFAULT_MON_THOUSANDS_SEP . "';";
	$str .= "var EWPP_DEFAULT_MON_DECIMAL_POINT='" . EWPP_DEFAULT_MON_DECIMAL_POINT . "';";
	$str .= "var EWPP_DEFAULT_FRAC_DIGITS=" . EWPP_DEFAULT_FRAC_DIGITS . ";";
	$str .= "var EWPP_MENUBAR_ROOTMENU_ID='" . EWPP_MENUBAR_ROOTMENU_ID . "';";
	$str .= "var EWPP_MENUBAR_ROOTCAT_ID='" . EWPP_MENUBAR_ROOTCAT_ID . "';";
	$str .= $PPLanguage->ToJSON(); // Language phrases
	echo $str;
}
...
class cpLanguage {
	...
	function XmlToJSON($XPath) {
		...
		foreach ($NodeList as $Node) {
			...
			$Str .= "\"" . ewpp_JsEncode2($Id) . "\":\"" . ewpp_JsEncode2($Value) . "\",";
		}
